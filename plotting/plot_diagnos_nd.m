function all_diagnos = plot_diagnos_nd(make_plots, do_filter, save_plots)
...
s2ms   = @(sec) datetime(0,0,0,0,0,sec, 'Format', 'mm:ss');
s2hms   = @(sec) datetime(0,0,0,0,0,sec, 'Format', 'HH:mm:ss');
...
fprintf('Most recent sim. time: %5.2f \n',sim_time(end))
...
fprintf('Avg. clock time per sim. timestep.: %s (D:H:M:S)\n',s2dhms(avg_clk_per_sim))
...
all_diagnos.diagnos.AvgClockTimePerSimTimestep = (avg_clk_per_sim);
...
gdpar.params.rho_0 = rho_0; gdpar.params.visco = visco;
...
for ii = 1:length(diffu_types)
    if isfield(params, diffu_types{ii})
        kappa(ii) = params.Re/params.(diffu_types{ii});
    else
        kappa(ii) = NaN;
    end
end

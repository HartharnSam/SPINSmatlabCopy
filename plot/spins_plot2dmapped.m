function pltinfo = spins_plot2dmapped(var, t_index, varargin)
% SPINS_PLOT2DMAPPED  Plot cross-section, averages and standard deviations of variables from a mapped grid.
%
%   pltinfo = spins_plot2dmapped(var,t_i) plots var at t_i
...
gd = gdpar.gd;
params = gdpar.params;
    %% make sure grid is full
gdnames = fieldnames(gd);
if isvector(gd.(gdnames{1}))
   error('spins_plot2d requires full grid for mapped grids.') 
end
